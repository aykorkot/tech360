  ...
  {block name='cart_voucher'}
    <div class="block-promo">
      <div class="cart-voucher js-cart-voucher">
        ...
        <h2 class="promo-code-title display-promo{if $cart.discounts|count > 0} with-discounts{/if}">
          ...
          <div class="promo-code">
            {block name='cart_voucher_form'}
              <form action="{$urls.pages.cart}" data-link-action="add-voucher" method="post">
                ...
                <button type="submit" class="btn btn-white"><span>{l s='Add' d='Shop.Theme.Actions'}</span></button>
              </form>
            {/block}
          ...
          <ul class="js-discount card-block promo-discounts">
            {foreach from=$cart.discounts item=discount}
              <li class="cart-summary-line">
                {l s='Profitez du code promo 👉' d='Shop.Theme.Actions'}
                <span class="value">
                  {$discount.code}  
                 {* {$discount.name} *}
                </span>
                {if $discount.reduction_percent > 0}
                  <span class="reduction">
                    (-{$discount.reduction_percent|intval}%)
                  </span>
                {elseif $discount.reduction_amount > 0}
                  <span class="reduction">
                    (-{$discount.reduction_amount|round:2} {$currency.sign})
                  </span>
                {/if}
              </li>
            {/foreach}
