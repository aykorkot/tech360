    ...
    public function getWidgetVariables(
        ...
        if (!empty($brands)) {
            foreach ($brands as &$brand) {
                ...
                $imageType = ImageType::getFormattedName('small'); // ou 'default', selon ce qui est configuré
                $imageFilename = $brand['id_manufacturer'] . '-' . $imageType . '.jpg';
                $imagePath = _PS_MANU_IMG_DIR_ . $imageFilename;
        
                if (file_exists($imagePath)) {
                    // Génère l'URL complète de l'image si elle existe
                    $brand['image'] = $this->context->link->getManufacturerImageLink($brand['id_manufacturer'], $imageType);
                } else {
                    // Image par défaut si aucune image spécifique n'existe
                    $brand['image'] = _THEME_MANU_DIR_ . $this->context->language->iso_code . '-default.jpg';
                }
            }
        }
