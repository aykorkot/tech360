...
class EgProduitPopulaireClass extends ObjectModel
{
    public $id;
    public $meta_title;
    public $description;  
    ...
    public static $definition = array(
        'table' => 'egproduitpopulaire',
        'primary' => 'id_egproduitpopulaire',
        'multilang' => true,
        'multilang_shop' => true,
        'fields' => array(
    ...
    public static function getAllByHook($hook,$id_shop = null)
    {
        ...
        $query = new DbQuery();
        $query->select('p.`id_egproduitpopulaire`');
        $query->from('egproduitpopulaire', 'p');
        $query->where('p.`active` =1');
        $query->where('p.`hook_names` = "'.$hook.'"');
    
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($query);
    }
    public static function getCategorySelectedById($id_egproduitpopulaire, $id_shop = null)
    {
        if (!$id_shop) {
            $id_shop = Context::getContext()->shop->id;
        } 
        ...
        $query->from('egproduitpopulaire', 'p');
        $query->where('p.`id_egproduitpopulaire` = '.(int) $id_egproduitpopulaire);
    ...
    {
        $query = new DbQuery();
        $query->select('ep.`id_egproduitpopulaire`, ep.`position`');
        $query->from('egproduitpopulaire', 'ep');
        ...
        foreach ($products as $product) {
            if ((int) $product['id_egproduitpopulaire'] == (int) $this->id) {
        ...
        return (Db::getInstance()->execute('
            UPDATE `'._DB_PREFIX_.'egproduitpopulaire`
            ...
            WHERE `id_egproduitpopulaire` = '.(int)$moved_product['id_egproduitpopulaire']));
    } 
}
